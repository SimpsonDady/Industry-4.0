        ...
        for data in self.load_data:
            ...
            for i in range(len(self.plan_data)):
                if self.plan_data[i].machine_name == data.machine_name:
                    self.match[-1] = i
            load = LoadData(data.date, data.time)
            self.status_format.append(load.build(data.work))
            print('     Status format: ' + data.machine_name)
            self.execute_format.append(load.build(data.code))
            print('     Execute format: ' + data.machine_name)
        for data in self.plan_data:
            plan = WorkList(data.start_date, data.start_time, data.end_date, data.end_time)
